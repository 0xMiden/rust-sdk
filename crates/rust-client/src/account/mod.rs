...
impl<AUTH> Client<AUTH> {
    ...
    /// public P2ID note with the native asset. The node answers when it submits the transaction
    /// that creates the note, before that transaction is committed. The note is not part of the
    /// response, and the client does not see it until a [`Client::sync_state`] runs after the note
    /// is committed. The client tracks the note tag of every account it owns, so that sync imports
    /// the note and [`Client::get_consumable_notes`] lists it. Sync again until the note arrives.
