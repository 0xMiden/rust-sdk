...
* [FEATURE][rust] `Client::sync_state` now fetches private notes from the Note Transport Layer concurrently with the on-chain state fetch, instead of running them one after the other. The two halves talk to different endpoints, so the sync costs one round trip instead of two. `Client::import_notes`, `Client::sync_note_transport`, `Client::fetch_private_notes`, `Client::ensure_genesis_in_place` and `Client::ensure_rpc_limits_in_place` now take `&self` instead of `&mut self`; existing call sites are unaffected.
* [FIX][rust] A private note delivered over the Note Transport Layer is now marked consumed in the same sync when it was already spent on chain. The transport delivers the note's metadata alongside its details, but the import discarded it and re-derived it from the chain instead — a lookup capped at the client's sync height, which silently found nothing for a note committed above it. With no metadata there is no nullifier, so the note was skipped by the nullifier check and stayed `Expected` until a later sync. The import now keeps the metadata the transport sent, and asks the node about every note whose nullifier it can compute rather than only the committed ones.
