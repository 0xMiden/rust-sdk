...
* [BREAKING][removal][rust] The note transport gRPC client now uses the service and message definitions from the node repository. `NoteTransportCursor` now stores the node's nonce and sequence, persisted scalar cursors are reset, and the unsupported `NoteTransportClient::stream_notes` API and `NoteStream` trait were removed ([#2594](https://github.com/0xMiden/rust-sdk/pull/2594)).
