...
pub struct TransactionRequest {
    ...
    /// Pinned consumption mode of selected input notes, set through
    /// [`TransactionRequestBuilder::explicit_input_notes`]. `Some(proof)` pins the note as
...
impl TransactionRequest {
    ...
    /// Returns the input notes whose consumption mode the request pins, each with the inclusion
    /// proof it is consumed with. A note paired with `None` is consumed as unauthenticated.
    ///
    /// The executing client infers the mode of every input note that is absent from this iterator
    /// from its store.
    pub fn explicit_input_notes(
        &self,
    ) -> impl Iterator<Item = (NoteId, Option<&NoteInclusionProof>)> {
        self.explicit_input_notes
            .iter()
            .map(|(note_id, proof)| (*note_id, proof.as_ref()))
    }

    /// Returns the IDs of the input notes whose consumption mode the executing client infers from
    /// its store.
    ...
    /// Returns the inclusion proofs of the input notes pinned as authenticated.
    ...
    pub(crate) fn build_input_notes(
        ...
        // Add the notes with a pinned mode.
        ...
        // Add the store-authenticated notes whose mode is inferred.
...
mod tests {
    ...
    use miden_protocol::note::{Note, NoteInclusionProof, NoteTag, NoteType};
    ...
    use crate::store::InputNoteRecord;
    use crate::store::input_note_states::CommittedNoteState;
    use crate::transaction::ForeignAccount;

    // HELPERS
    // --------------------------------------------------------------------------------------------

    fn mock_note(seed: u64) -> Note {
        let sender_id = AccountId::try_from(ACCOUNT_ID_SENDER).unwrap();
        let target_id =
            AccountId::try_from(ACCOUNT_ID_REGULAR_PUBLIC_ACCOUNT_IMMUTABLE_CODE).unwrap();
        let faucet_id = AccountId::try_from(ACCOUNT_ID_PRIVATE_FUNGIBLE_FAUCET).unwrap();
        let mut rng = RandomCoin::new([seed, 1, 2, 3].map(Felt::new_unchecked).into());
    ...
    /// Returns a proof that places a note in `block_num`. Classification never verifies the proof,
    /// so an empty path is enough.
    fn inclusion_proof(block_num: u32) -> NoteInclusionProof {
        let note_path = SparseMerklePath::from_parts(0, Vec::new()).unwrap();
        NoteInclusionProof::new(BlockNumber::from(block_num), 0, note_path).unwrap()
    }

    /// Returns a store record that tracks `note` as authenticated.
    fn authenticated_record(note: &Note) -> InputNoteRecord {
        let state = CommittedNoteState {
            metadata: *note.metadata(),
            inclusion_proof: inclusion_proof(1),
            block_note_root: EMPTY_WORD,
        };
        InputNoteRecord::new(note.clone().into(), note.attachments().clone(), None, state.into())
    }
    ...
    #[test]
    fn build_input_notes_keeps_pinned_modes_and_infers_the_rest() {
        let inferred_tracked = mock_note(0);
        let inferred_untracked = mock_note(1);
        let pinned_unauthenticated = mock_note(2);
        let pinned_authenticated = mock_note(3);
        let pinned_proof = inclusion_proof(7);

        let tx_request = TransactionRequestBuilder::new()
            .input_notes([(inferred_tracked.clone(), None), (inferred_untracked.clone(), None)])
            .explicit_input_notes([
                (InputNote::unauthenticated(pinned_unauthenticated.clone()), None),
                (
                    InputNote::authenticated(pinned_authenticated.clone(), pinned_proof.clone()),
                    None,
                ),
            ])
            .build()
        ...
        let inferred: Vec<_> = tx_request.inferred_input_note_ids().collect();
        assert_eq!(inferred, vec![inferred_tracked.id(), inferred_untracked.id()]);
        let pinned: BTreeMap<_, _> = tx_request.explicit_input_notes().collect();
        assert_eq!(
            pinned,
            BTreeMap::from([
                (pinned_unauthenticated.id(), None),
                (pinned_authenticated.id(), Some(&pinned_proof)),
            ])
        );

        // The store tracks one inferred note and the note pinned as unauthenticated. It does not
        // track the note pinned as authenticated.
        let records = vec![
            authenticated_record(&inferred_tracked),
            authenticated_record(&pinned_unauthenticated),
        ];
        let input_notes = tx_request.build_input_notes(records).unwrap();

        assert_eq!(input_notes.num_notes(), 4);
        assert!(matches!(input_notes.get_note(0), InputNote::Authenticated { .. }));
        assert_eq!(input_notes.get_note(0).id(), inferred_tracked.id());
        assert_eq!(input_notes.get_note(1), &InputNote::unauthenticated(inferred_untracked));
        assert_eq!(input_notes.get_note(2), &InputNote::unauthenticated(pinned_unauthenticated));
        assert_eq!(
            input_notes.get_note(3),
            &InputNote::authenticated(pinned_authenticated, pinned_proof)
        );
    }

    #[test]
    fn deserialization_rejects_a_pinned_note_the_request_does_not_consume() {
        let consumed = mock_note(0);
        let not_consumed = mock_note(1);

        let mut tx_request = TransactionRequestBuilder::new()
            .input_notes([(consumed, None)])
            .build()
            .unwrap();
        tx_request.explicit_input_notes.insert(not_consumed.id(), None);

        let result = TransactionRequest::read_from_bytes(&tx_request.to_bytes());
