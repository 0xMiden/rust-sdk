...
* [FIX][rust] A `SyncNotes` response whose note claims a different commit height than the block it was returned in is now rejected with `RpcError::InvalidResponse` while parsing the response. The two are independent fields, so a node contradicting itself is caught before any sync path consumes the note ([#2400](https://github.com/0xMiden/rust-sdk/issues/2400)).
