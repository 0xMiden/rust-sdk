...
* [FEATURE][rust] Added `Client::track_account_witness`, `Client::untrack_account_witness` and `Client::tracked_account_witnesses` to register accounts whose account witness is refreshed on every sync and cached in the store (new `account_witnesses` table). A transaction using a registered account as a `ForeignAccount::Private` resolves the witness locally instead of issuing a `GetAccount` request, which moves the cost from once per transaction to once per sync. A cached witness is only used when it was fetched at the transaction's reference block, since it opens under that block's account root ([#2443](https://github.com/0xMiden/rust-sdk/issues/2443)).
