...
* [BREAKING][param][rust] Settings are namespaced by a domain. `Client::set_setting`, `Client::get_setting`, `Client::remove_setting`, `Client::list_setting_keys` and their `Store` counterparts, plus `Store::apply_settings_mutations`, take a `&SettingDomain` as their first argument. Build one with `SettingDomain::new(name)`, which rejects an empty name with the new `StoreError::EmptySettingDomain`. Every domain built that way is `SettingScope::User`; the client's own domains are `SettingScope::Client` and cannot be built from outside the crate, so a caller can no longer overwrite or read the client's internal state through this API ([#2456](https://github.com/0xMiden/rust-sdk/pull/2456)).
* [BREAKING][param][store] `Store::list_setting_keys` returns only the keys of the domain it is given, and the new `Store::list_user_setting_domains` (exposed as `Client::list_setting_domains`) returns the user-owned domains that hold at least one key. Client-owned domains are never listed, so the client's internal keys no longer reach the caller. `Store` implementations must add the new method ([#2456](https://github.com/0xMiden/rust-sdk/pull/2456)).
* [BREAKING][behavior][store] The SQLite `settings` table now carries `scope` and `domain` columns with `(scope, domain, name)` as its primary key. This changes the schema fingerprint, so opening a database created before this change fails with `SchemaDrift` and existing stores must be recreated ([#2456](https://github.com/0xMiden/rust-sdk/pull/2456)).
