...
* [FEATURE][rust] Added `Client::track_account_witness`, `Client::untrack_account_witness` and `Client::tracked_account_witnesses` to register accounts whose account witness is refreshed on every sync and cached in the store (new `account_witnesses` table). A transaction using a registered account as a foreign account resolves its inputs without a `GetAccount` request, which moves the cost from once per transaction to once per sync: a `ForeignAccount::Private` takes the witness from the store, and a `ForeignAccount::Public` the client tracks builds its whole `AccountInputs` from the store. A cached witness is only used when it was fetched at the transaction's reference block, since it opens under that block's account root, and for a public account only when the stored state still hashes to the commitment the witness proves ([#2443](https://github.com/0xMiden/rust-sdk/issues/2443)).
