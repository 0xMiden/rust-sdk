    ...
    AUTH: TransactionAuthenticator + Sync + 'static,
{
    ...
    /// Like [`Client::execute_program`], but routes the executed script's MASM debug output (the
    /// `miden::core::debug::print_*` procedures) to the writer `W` instead of stdout, which is a
    /// no-op on `wasm32-unknown-unknown`. `W` is default-constructed per execution.
    ///
    /// Unlike the default host, this also routes the advice-stack and advice-map printers, which
    /// can expose witness data. Use it only where that data is already the caller's own.
    #[cfg(feature = "debug-output")]
    ...
    /// Like [`Client::execute_transaction`], but routes MASM debug output to the writer `W` (see
    /// [`Client::execute_program_with_debugger`]).
    #[cfg(feature = "debug-output")]
