...
* [BREAKING][behavior][rust,cli] The client now gets its protocol configuration from the node during `Client::sync_state` instead of being given one. `SyncChainMmr` carries the configuration when the client syncs from genesis or when the configuration commitment changed over the synced range, and the client verifies it against the block header before storing it.
* [BREAKING][removal][rust] Removed `ClientBuilder::protocol_config` and `Client::add_protocol_config`. A client gets its configurations by syncing, so there is no longer a way to supply one.
* [BREAKING][removal][test] `Client::seed_protocol_config` replaces `Client::add_protocol_config` for a client that cannot reach a node, such as one backed by a mock chain. It is only available under the `testing` feature.
* [BREAKING][removal][cli] Removed the `MIDEN_PROTOCOL_CONFIG` environment variable and the `fee_faucet_id` setting. Both existed to supply a protocol configuration the node did not serve, and the CLI, benchmarks and integration tests no longer read them.
* [BREAKING][param][rust,store] `StateSyncUpdate` carries the protocol configuration a sync returned, so `StateSyncUpdate::from_parts` takes an extra `Option<ProtocolConfig>` and `into_parts` returns one. A `Store` implementation must persist it under `protocol_config_setting_key` in `SettingScope::Client`.
* [BREAKING][type][rust] `ChainMmrInfo` gained a `protocol_config: Option<ProtocolConfig>` field, so code that builds one must set it. A returned configuration whose commitment does not match the accompanying block header is rejected with `RpcError::InvalidResponse`.
