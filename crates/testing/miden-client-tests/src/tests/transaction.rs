...
async fn discarded_transaction_releases_its_input_notes() {
    ...
    // Consume the note with a transaction that is applied locally but never reaches the node, so
    // it goes stale once the chain moves past the discard delta.
    let request = TransactionRequestBuilder::new().build_consume_notes(vec![note.clone()]).unwrap();
