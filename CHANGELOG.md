...
* [FEATURE][rust] `Client::sync_state` now issues its independent gRPC calls concurrently instead of one after another, cutting the round trips a sync costs. `sync_notes` (with its `get_notes_by_id` follow-up) overlaps `sync_transactions`, the per-account `get_account` fetches overlap each other, and the limit-bounded chunk requests inside `sync_notes`, `sync_transactions`, `sync_nullifiers` and `get_notes_by_id` are issued in parallel, at most four in flight per call. The number and content of the requests is unchanged, as is the order in which their responses are applied ([#2361](https://github.com/0xMiden/rust-sdk/issues/2361)).
* [FEATURE][rust] `MockRpcApi` now counts the calls it serves per endpoint, exposed through `call_count` and `reset_call_counts`, so tests can assert how many round trips a client operation costs ([#2361](https://github.com/0xMiden/rust-sdk/issues/2361)).
