...
* [BREAKING][type][rust] Added the `ClientError::ChainAnchorError` and `ClientError::AllInputNotesScreenedOut` variants, so exhaustive matches on `ClientError` must handle them ([#2421](https://github.com/0xMiden/rust-sdk/pull/2421)).
* [BREAKING][behavior][rust] A request built with `ignore_invalid_input_notes` whose input notes are *all* screened out now fails with `ClientError::AllInputNotesScreenedOut` instead of an opaque note-count error from the consumption checker. This applies to every path that screens notes, including `BatchBuilder::push`. The flag drops the notes that cannot be consumed, not every note; a transaction consuming nothing still costs a proof and a fee to submit ([#2421](https://github.com/0xMiden/rust-sdk/pull/2421)).
