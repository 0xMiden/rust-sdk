...
When the network operator runs a funding service, the node pays the registered account a public note with the native asset, and the command returns as soon as the node submits the transaction that creates that note. The note is not committed on chain yet at that point, so it can take a few blocks to arrive. The client tracks the note tag of every account it owns, so a sync that runs after the note is committed imports it. Sync until the note is listed as consumable. Consuming it creates the account on chain, and the fee of that transaction is paid out of the received funds:
