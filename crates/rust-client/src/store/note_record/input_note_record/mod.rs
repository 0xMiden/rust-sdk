...
impl InputNoteRecord {
    ...
    /// Modifies the state of the note record to reflect that the transaction consuming the note
    /// was discarded, making the note available again. Only a note being processed by that
    /// transaction is affected. Returns `true` if the state was changed.
