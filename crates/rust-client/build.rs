//! Generates the Rust protobuf bindings for the node RPC, the note transport and the remote prover
//! clients. The generation runs only with the `tonic` feature, which is the only feature that
...
mod codegen {
    ...
    ///
    /// The node's descriptors embed the object schemas they import. These paths make prost
    /// reference the types `miden-objects` already defines, so the conversions it ships apply to
    /// them.
    ...
    fn compile_tonic_remote_prover_proto(out_dir: &Path) -> Result<(), Box<dyn Error>> {
        ...
        fs::create_dir_all(&std_out)?;
        fs::create_dir_all(&nostd_out)?;
    ...
    fn compile_tonic_note_transport_proto(out_dir: &Path) -> Result<(), Box<dyn Error>> {
    ...
    fn compile_tonic_client_proto(out_dir: &Path) -> Result<(), Box<dyn Error>> {
    ...
    fn generate_wrapper(
        ...
        fs::write(out_dir.join(wrapper_name), wrapper)?;
    ...
    fn replace_no_std_types_in_dir(dir: &Path) -> Result<(), Box<dyn Error>> {
