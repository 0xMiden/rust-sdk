...
* [BREAKING][behavior][rust,cli] `FilesystemKeyStore` now encrypts its key files with a password. `FilesystemKeyStore::new` and `ClientBuilder::filesystem_keystore` take a `password` argument. The previous plaintext behavior of `new` moved to `FilesystemKeyStore::new_plaintext`, which is only recommended for development. `KeyStoreError` gained the `EncryptionError` and `InvalidPassword` variants. The CLI encrypts the keystore by default and reads the password from `MIDEN_KEYSTORE_PASSWORD` or a terminal prompt; `init --plaintext-keystore` or `keystore_encrypted = false` opt out. An existing plaintext keystore refuses to open until `keys --encrypt` encrypts it or the field is set to `false` ([#30](https://github.com/0xMiden/rust-sdk/issues/30)).
