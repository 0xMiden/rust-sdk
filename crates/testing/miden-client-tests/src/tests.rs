...
/// The expiration delta must bound every request it is set on, not only those that create notes.
/// A consume request has no output notes and therefore no `SendNotes` script, so the delta has to
/// be applied through a dedicated expiration script.
#[tokio::test]
async fn expiration_delta_applies_to_request_without_own_output_notes() {
    ...
    let transaction_result =
        Box::pin(client.execute_transaction(wallet.id(), transaction_request)).await.unwrap();
