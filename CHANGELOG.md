...
* [FIX][cli] The asset table printed by `account --show` now identifies each non-fungible asset by its asset ID, so two non-fungible assets issued by the same faucet are no longer displayed identically ([#2374](https://github.com/0xMiden/rust-sdk/pull/2374)).
