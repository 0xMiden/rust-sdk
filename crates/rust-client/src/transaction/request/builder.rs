...
pub struct TransactionRequestBuilder {
    /// Notes to be consumed by the transaction. Notes whose inclusion proof is present in the store
    /// are will be consumed as authenticated; the ones that do not have proofs will be consumed as
    /// unauthenticated.
    input_notes: Vec<Note>,
    /// Optional arguments of the Notes to be consumed by the transaction. This includes both
    /// authenticated and unauthenticated notes.
    input_notes_args: Vec<(NoteId, Option<NoteArgs>)>,
    /// Notes to be created by the transaction. The full note data is needed internally to build the
    ...
    /// the network, and injected as advice inputs. Additionally, the account's code will be added
    /// to the executor and prover.
...
impl TransactionRequestBuilder {
    ...
    /// Sets an optional [`Word`] that will be pushed to the stack for the authentication procedure
    ...
    pub fn build_pswap_consume(
        ...
        // expected recipients so the transaction is validated against them, but don't register them
        // as expected future notes — that's the creator's concern, and doing so here would leave
        // stale, un-consumable notes in the consumer's store.
