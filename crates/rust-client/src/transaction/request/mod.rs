...
pub struct TransactionRequest {
    /// Notes to be consumed by the transaction. includes both authenticated and unauthenticated
    /// notes. Notes which ID is present in the store are considered authenticated, the ones which
    /// ID is does not exist are considered unauthenticated.
...
impl TransactionRequest {
    ...
    ///
    /// Authenticated input notes are provided by the caller (typically fetched from the store). Any
    /// requested notes not present in that authenticated set are treated as unauthenticated. The
    /// transaction input notes will include both authenticated and unauthenticated notes in the
