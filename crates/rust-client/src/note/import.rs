    ...
    AUTH: TransactionAuthenticator + Sync + 'static,
{
    ...
    async fn import_note_records_by_details(
        ...
        // A note found committed below the sync height may have been spent below it too, where the
        // chain sync never looks for nullifiers.
        self.mark_spent_notes(&mut note_records).await?;

        Ok(note_records)
    }

    /// Marks as consumed every record in `note_records` whose nullifier is already on chain.
    ///
    /// The query starts at the lowest commitment block among the records. Records without an
    /// inclusion proof have no nullifier to ask about and are skipped.
    async fn mark_spent_notes(
        &self,
        ...
        for note_record in note_records.iter_mut() {
            if let Some(nullifier) = note_record.nullifier()
                && let Some(Some(spent_at)) = spent_heights.get(&nullifier)
            {
