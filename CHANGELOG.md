...
* [rust] `Client::sync_state` fetches from the Note Transport Layer and from the node concurrently, then imports the delivered private notes and finishes the chain sync, instead of running a full note transport sync before the chain sync ([#2453](https://github.com/0xMiden/rust-sdk/pull/2453)).
...
* [FIX][rust] An expected note imported with `Client::import_notes`, including one fetched from the Note Transport Layer, whose nullifier is already on chain is now imported as consumed instead of committed, so `get_consumable_notes` no longer reports notes the node will reject ([#2453](https://github.com/0xMiden/rust-sdk/pull/2453)).
