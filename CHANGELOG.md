...
* [FEATURE][rust] `Client::sync_state` now fetches from the Note Transport Layer and from the node concurrently instead of one after the other. Neither fetch writes to the store; both results are written afterwards, in one order — the chain update first, then the note import — so a failure on either side leaves the store untouched. The import runs against the sync height the chain update established, which lets a private note delivered over the transport settle in the same call even when it was committed inside that sync's own window.
...
* [FIX][rust] Notes imported from their details — the path the Note Transport Layer uses — are now checked against the chain's nullifiers and recorded as consumed when they were already spent, instead of staying committed and being offered as consumable forever. The search starts at the earliest block the note could have been committed in, so it reaches the same history the commitment scan does, and the transport hands the import the note header's metadata so the nullifier can be computed without waiting to recover it from the chain. Spends above the client's sync height are left to the chain sync, which can also attribute the consuming account. Notes already stuck in the store are only repaired if the transport delivers them again ([#2422](https://github.com/0xMiden/rust-sdk/issues/2422)).
