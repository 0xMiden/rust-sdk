...
* [BREAKING][behavior][rust] Notes fetched from the Note Transport Layer under an account-derived tag are screened before they are stored. The notes that are not consumable by any of the tracked accounts are discarded. Notes under a tag registered by `Client::add_note_tag` or by a subscription are kept unscreened ([#2474](https://github.com/0xMiden/rust-sdk/pull/2474)).
