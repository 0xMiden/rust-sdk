...
* [FEATURE][rust] Added the `store::forest_backend` module. `RowForestBackend` implements the account SMT forest storage backend on top of a small synchronous `ForestRowStore` row interface, so alternative stores (such as an IndexedDB store) can reuse the shared Merkle computation by implementing only the row reads and writes. The SQLite forest backend is now an implementation of this interface (#TBD).
