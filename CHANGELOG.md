...
* [FIX][rust] `VerifyingRpcClient` now rejects range-scoped responses that carry data stamped outside the requested `block_from`/`block_to` window, with `RpcError::InvalidResponse`. This covers `sync_notes`, `sync_nullifiers`, `sync_storage_maps`, `sync_account_vault` and `sync_transactions`, whose block windows were previously forwarded to the node and never checked, so a node could answer a query about one block range with data from another ([#2503](https://github.com/0xMiden/rust-sdk/pull/2503)).
