...
* [testing] Added an opt-in fee-charging mode to the test node's genesis: setting `MIDEN_TEST_NODE_VERIFICATION_BASE_FEE` to a non-zero value emits that `verification_base_fee` and, with it, two public wallets holding 5,000 MIDEN each that `scripts/start-test-node.sh` exposes under `./data` as `wallet_<n>.mac`. Fees stay off when the variable is unset or `0`, leaving existing test balances untouched. See the crate README for why the wallets are there and what a fee-charging node still needs before the integration suite can run against it.
* [testing] Added `MIDEN_TEST_NODE_USE_INSTALLED` to `scripts/start-test-node.sh`, which runs whatever node binaries are already installed under `target/test-node/install` instead of checking them against the pinned rev. This makes it possible to test against a node built from a patched checkout, which a `cargo install` of the pin would otherwise overwrite; the script aborts rather than falling back to the pin when a binary is missing.
