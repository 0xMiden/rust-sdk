...
* [BREAKING][arch][rust,rpc] Protocol object messages now come from the canonical `miden-objects` schemas instead of node-owned copies. The client applies `miden_objects::EXTERN_PATHS` when generating its bindings, so `proto::account`, `proto::asset`, `proto::blockchain`, `proto::note`, `proto::primitives`, `proto::protocol_config` and `proto::transaction` resolve to `miden_objects::proto`. Requires a node that speaks the canonical schemas.
* [BREAKING][removal][rust,rpc] Removed the client's hand-written protobuf conversions for protocol objects, which `miden-objects` now provides: the `rpc::domain` modules `block`, `digest`, `merkle` and `smt` are gone, along with the `AccountId`, `AccountHeader`, `AccountStorageHeader`, `AccountWitness`, `NoteId`, `NoteMetadata`, `NoteHeader`, `NoteInclusionProof`, `NoteScript`, `TransactionId` and `Asset` conversions in the surviving modules. `primitives.Digest` is now `primitives.Word`.
* [BREAKING][type][rust] `RpcConversionError` gained a `CanonicalConversion` variant that wraps `miden_objects::ConversionError`, so exhaustive matches on it must handle the new variant.
