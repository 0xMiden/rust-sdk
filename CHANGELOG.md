...
* [BREAKING][rust] Transaction fees moved out of the kernel epilogue and into the authentication procedure. On a chain whose `verification_base_fee` is non-zero, signature-based auth components (`AuthSingleSig`, `AuthMultisig`, `AuthGuardedMultisig`) require the transaction to commit fee conversion info, and the paying account must hold a balance of the fee asset. For `AuthSingleSig` the client commits it for you (see the enhancement below); an `AuthMultisig` or `AuthGuardedMultisig` caller has to commit its own, because there the commitment doubles as the replay salt. Use the new `TransactionRequestBuilder::fee_conversion_info` in either case to pay in a different asset. Chains that charge no fee are unaffected.
...
* [BREAKING][serialization][rust] `TransactionRequest`'s serialized form carries an extra flag recording whether its auth arg was set through `TransactionRequestBuilder::fee_conversion_info`. The format is unversioned, so a request serialized by an earlier version does not deserialize with this one ([#2446](https://github.com/0xMiden/rust-sdk/issues/2446)).
* [BREAKING][type][rust] Added the `TransactionRequestError::FeeConversionInfoRequired` variant, rejecting an `AuthMultisig` or `AuthGuardedMultisig` request that declares no fee conversion info on a fee-charging chain, which would otherwise abort inside `fee::pay_fee` ([#2446](https://github.com/0xMiden/rust-sdk/issues/2446)).
* [BREAKING][type][test] `miden_client::testing::common::TestClient` is a struct wrapping `Client<FilesystemKeyStore>` rather than an alias for it, so it can carry the fee funder its account helpers draw from. It derefs to the client, so method calls are unaffected, but a `Client` no longer passes where a `TestClient` is expected. `TestClient::chain_charges_fees` takes `&mut self`, since it fetches genesis when the store has not seen it ([#2446](https://github.com/0xMiden/rust-sdk/issues/2446)).
* [BREAKING][test] The `miden_client::testing::common` account helpers (`insert_new_wallet`, `insert_new_wallet_with_seed`, `insert_new_fungible_faucet`, `insert_account_with_custom_component`) return `anyhow::Result` instead of `Result<_, ClientError>`, because funding an account can fail for reasons a `ClientError` cannot express. `insert_new_fungible_faucet` also installs `BasicWallet` alongside the faucet component, so the faucet can receive the fee asset that pays for its own transactions, which changes the code commitment of the accounts it builds. `assert_account_has_single_asset` is renamed to `assert_account_balance`, which is what it checked ([#2446](https://github.com/0xMiden/rust-sdk/issues/2446)).
...
* [FEATURE][rust] Added `AuthGuardedMultisig`, `AuthGuardedMultisigConfig`, `GuardianConfig` and `ApproverSet` to `miden_client::auth`, which previously exposed only the single- and multisig components. Building a guarded multisig account no longer means reaching past the client into `miden_standards` ([#2446](https://github.com/0xMiden/rust-sdk/issues/2446)).
...
* [FIX][test] The integration tests now run against a fee-charging chain. The testing node's genesis charges a fee by default (`MIDEN_VERIFICATION_BASE_FEE`, default `500`), generates the native fee faucet itself so the accounts it deploys can be seeded with that asset, and pre-funds a pool of basic wallets the suite draws from via a new `--funders` argument (`MIDEN_FUNDER_ACCOUNTS`). Accounts created by the `miden_client::testing::common` helpers are funded and deployed automatically, and `TestClient::deploy_account` does the same for accounts a test builds itself. The AggLayer accounts are consequently always part of genesis (the `AGGLAYER_GENESIS` env var and the `start-node-agglayer` target are gone) and the AggLayer tests load them from `AGGLAYER_ACCOUNTS_DIR` ([#2446](https://github.com/0xMiden/rust-sdk/issues/2446)).
* [FIX][rust] Note screening now commits the chain's native fee conversion info for the auth components that read it, so notes are no longer reported as unconsumable on a fee-charging chain. A note whose script the consumption checker cannot classify statically — a SWAP note, or any custom script — is screened by trial-executing it, which runs the account's auth procedure, and with no conversion info committed that aborts in `fee::pay_fee`. The checker reports the abort as unconsumable conditions, so such notes were dropped as irrelevant during sync for `AuthSingleSig`, `AuthMultisig` and `AuthGuardedMultisig` accounts. P2ID and P2IDE notes were unaffected, being classified without execution ([#2446](https://github.com/0xMiden/rust-sdk/issues/2446)).
