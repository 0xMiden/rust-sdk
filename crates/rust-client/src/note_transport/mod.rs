...
impl<AUTH> Client<AUTH> {
    ...
    /// deserialization failure an empty set is returned as well: re-treating every tracked tag as
    /// new only triggers a one-off backfill, which dedupes, whereas failing here would fail every
    /// subsequent sync. The unreadable bytes are left in place rather than deleted, so that this
    /// stays a pure read; the next covered-set write replaces them.
    async fn load_covered_tags(&self) -> Result<BTreeSet<NoteTag>, ClientError> {
        ...
        match BTreeSet::<NoteTag>::read_from_bytes(&bytes) {
            Ok(tags) => Ok(tags),
            Err(err) => {
                tracing::warn!(?err, "unreadable covered-tags set; treating it as empty");
                Ok(BTreeSet::new())
            },
        }
    }
    ...
    AUTH: TransactionAuthenticator + Sync + 'static,
{
    ...
    /// a sender-provided block hint. Scanning back from the sync height handles the race where a
    /// note is committed on-chain just before the NTL delivers its data. Without it the import
    /// would scan forward from the sync height and miss the already-committed note. A
    ...
    /// observe the other's writes, and the loser of a failed join is dropped mid-await, so a
    /// cancelled fetch leaves nothing behind.
    ///
    /// Returns `None` when note transport is disabled.
    pub(crate) async fn fetch_transport_update(
        &self,
    ) -> Result<Option<TransportUpdate>, ClientError> {
        if !self.is_note_transport_enabled() {
            return Ok(None);
        }

        // The import runs after the chain update is applied, when the sync height is already at
    ...
    /// hint: [`Self::NOTE_LOOKBACK_BLOCKS`] below the current sync height.
    async fn transport_lookback_floor(&self) -> Result<BlockNumber, ClientError> {
        let sync_height = self.get_sync_height().await?;
        Ok(BlockNumber::from(
            sync_height.as_u32().saturating_sub(Self::NOTE_LOOKBACK_BLOCKS),
        ))
    }
    ...
    ///
    /// The pages are accumulated rather than imported as they arrive, so that the whole drain
    /// stays free of store writes.
    async fn fetch_backfill_pages(
        &self,
        tag: NoteTag,
    ) -> Result<Vec<FetchedTransportNote>, ClientError> {
        let mut fetched = Vec::new();
        let mut cursor = NoteTransportCursor::init();
        for _ in 0..Self::MAX_BACKFILL_ITERATIONS {
            let (notes, new_cursor) = self.fetch_transport_batch(cursor, &[tag]).await?;
            fetched.extend(notes);
    ...
    ///
    /// The server paginates; this method issues one RPC and returns the fetched notes together
    /// with the new cursor. The returned cursor equals the input cursor when the batch was empty
    /// (i.e. no new notes). Callers that want to drain a tag's full backlog should loop until
    /// `new_cursor == cursor` (see `Client::fetch_backfill_pages`). Callers that do
    ...
    ///
    /// Neither the notes nor the cursor are persisted here; see
    /// `Client::import_transport_notes`.
    pub(crate) async fn fetch_transport_batch(
        &self,
        cursor: NoteTransportCursor,
        tags: &[NoteTag],
    ) -> Result<(Vec<FetchedTransportNote>, NoteTransportCursor), ClientError> {
        let (note_infos, rcursor) =
            self.get_note_transport_api()?.fetch_notes(tags, cursor).await?;

        let mut notes = Vec::with_capacity(note_infos.len());
        for note_info in &note_infos {
            ...
            notes.push(FetchedTransportNote {
                details: note.into(),
                // The header carries the attachment-aware metadata; the rejoined note has empty
                // attachments, and both the note id and the nullifier commit to the attachments
                // commitment, so neither can be derived from the rejoined note.
                metadata: *note_info.header.metadata(),
                note_id: note_info.header.id(),
                block_hint: note_info.block_hint,
            });
        }
...
#[derive(Debug)]
pub(crate) struct FetchedTransportNote {
    /// Rejoined note details. [`rejoin_note`] drops attachments, so these determine neither the
    /// on-chain note id nor the nullifier on their own.
    details: NoteDetails,
    /// Metadata as it came on the note header.
    metadata: NoteMetadata,
    /// On-chain note id, taken from the header.
    note_id: NoteId,
    /// Sender-provided block hint, when the sender relayed one.
    block_hint: Option<BlockNumber>,
}
