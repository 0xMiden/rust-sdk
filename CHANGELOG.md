...
* [FEATURE][cli] Added `completions <shell>` to print a shell completion script to stdout for bash, zsh, fish, powershell or elvish. The script is generated at runtime from the CLI definition and picks up the binary's actual invoked name, so it stays correct under an alias such as [Midenup](https://github.com/0xMiden/midenup) ([#408](https://github.com/0xMiden/rust-sdk/issues/408)).
