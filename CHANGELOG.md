...
* [BREAKING][type][rust] `RpcConversionError` gained a `CanonicalConversion` variant that wraps `miden_objects::ConversionError`, and lost the `DeserializationError`, `NotAValidFelt`, `NoteTypeError`, `MerkleError` and `InvalidInt` variants, which the removed conversions were the only source of ([#2549](https://github.com/0xMiden/rust-sdk/pull/2549)).
