...
* [BREAKING][removal][test] `Client::seed_protocol_config` replaces `Client::add_protocol_config` for a client that cannot reach a node, such as one backed by a mock chain. It is only available under the `testing` feature ([#2591](https://github.com/0xMiden/rust-sdk/pull/2591)).
* [BREAKING][removal][cli] Removed the `MIDEN_PROTOCOL_CONFIG` environment variable and the `fee_faucet_id` setting. Both existed to supply a protocol configuration the node did not serve, and the CLI, benchmarks and integration tests no longer read them ([#2591](https://github.com/0xMiden/rust-sdk/pull/2591)).
* [BREAKING][param][rust,store] `StateSyncUpdate` carries the protocol configuration a sync returned, so `StateSyncUpdate::from_parts` takes an extra `Option<ProtocolConfig>` and `into_parts` returns one. A `Store` implementation must persist it under `protocol_config_setting_key` in `SettingScope::Client` ([#2591](https://github.com/0xMiden/rust-sdk/pull/2591)).
* [BREAKING][type][rust] `ChainMmrInfo` gained a `protocol_config: Option<ProtocolConfig>` field, so code that builds one must set it. A returned configuration whose commitment does not match the accompanying block header is rejected with `RpcError::InvalidResponse` ([#2591](https://github.com/0xMiden/rust-sdk/pull/2591)).
