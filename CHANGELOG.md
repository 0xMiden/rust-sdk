...
* [BREAKING][removal][rust] The `miden_client::crypto::RandomCoin` re-export is gone; the client no longer uses `RandomCoin` anywhere. Use a `rand` CSPRNG such as `ChaCha20Rng`, and the new `miden_client::crypto::draw_felt` / `draw_word` helpers where a random `Felt` or `Word` is needed from a generator that does not implement `FeltRng` (for example a note serial number: `.serial_number(draw_word(&mut rng))` in place of `.generate_serial_number(&mut rng)`). The `FeltRng` re-export remains, and `Client::rng` still implements it ([#2204](https://github.com/0xMiden/rust-sdk/issues/2204)).
* [BREAKING][behavior][rust] The client now keeps two random number generators. `Client::rng` covers note serial numbers, script arguments and account seeds, and is the one `ClientBuilder::rng` overrides; the new `Client::secure_rng` covers secret keys, is always seeded from the operating system and cannot be overridden. Transaction input sealing draws its ephemeral key and nonce from the secure generator, so seeding `ClientBuilder::rng` for a reproducible run can no longer replay a sealing nonce or a key. Callers generating secret keys must switch from `client.rng()` to `client.secure_rng()` ([#2204](https://github.com/0xMiden/rust-sdk/issues/2204)).
* [BREAKING][type][rust] `ClientBuilder::rng` now requires `CryptoRng + Send + Sync`, the marker trait `ClientFeltRng` is renamed to `ClientCryptoRng` with its bound changed from `FeltRng` to `CryptoRng`, and a client with no RNG configured now defaults to an OS-seeded `ChaCha20Rng` instead of a `RandomCoin`. `RandomCoin` is no longer accepted, since it is not a `CryptoRng`; pass a `rand` CSPRNG or drop the `rng()` call to take the default. `Client::rng()` still returns a `FeltRng`, so `draw_word` and `generate_serial_number` call sites are unaffected. For reproducible randomness, seed a `ChaCha20Rng` with `from_seed`; note that `CryptoRng` constrains the generator's algorithm and not the entropy of its seed ([#2204](https://github.com/0xMiden/rust-sdk/issues/2204)).
