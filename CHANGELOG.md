...
* [BREAKING][type][rust] `PublicAccountUpdate` was replaced by `AccountStateUpdate`, defined in `miden_client::store`. It is a struct carrying the new account header, a `StorageUpdate` and a `VaultUpdate`. Each part is either `Full` (the state the `get_account` response carried, which the store applies as a replacement) or `Patch` (the changes fetched over the synced range for a part the node reported as oversized). Syncing a public account calls `sync_storage_maps` only when a storage map is oversized and `sync_account_vault` only when the vault is, instead of fetching both whenever any part was oversized. `Store` implementations must apply each part according to its variant. `AccountUpdates` is now defined once, in `miden_client::store`, and is no longer re-exported from `miden_client::sync` ([#2217](https://github.com/0xMiden/rust-sdk/issues/2217)).
