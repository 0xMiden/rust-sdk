...
* [BREAKING][rust] Transaction fees moved out of the kernel epilogue and into the authentication procedure. On a chain whose `verification_base_fee` is non-zero, signature-based auth components (`AuthSingleSig`, `AuthMultisig`) require the transaction to commit fee conversion info, and the paying account must hold a balance of the fee asset. The client commits it for you (see the enhancement below); use the new `TransactionRequestBuilder::fee_conversion_info` to pay in a different asset. Chains that charge no fee are unaffected.
...
* [FIX][rust] An empty auth argument no longer suppresses the fee conversion info the client attaches, and an account whose auth component reads that argument as a caller-chosen salt (`AuthMultisig`, `AuthGuardedMultisig`) is now rejected with `TransactionRequestError::FeeConversionInfoRequired` instead of failing inside the VM. Accounts carrying an auth component the client cannot classify are left alone rather than panicking. Added `TransactionRequest::with_auth_arg` to attach a commitment the caller computed itself, without the component classification that `TransactionRequestBuilder::fee_conversion_info` implies ([#2446](https://github.com/0xMiden/rust-sdk/issues/2446)).
