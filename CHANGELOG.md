...
* [BREAKING][behavior][rust] Notes fetched from the Note Transport Layer are screened before they are stored: a note that no tracked account can consume is discarded instead of imported. A tag only matches the 14 most significant bits of an account ID prefix, so a fetch can return notes addressed to other accounts. A client that tracks a tag but has no account able to consume the notes carrying it now stores nothing, and the transport cursor still advances past the discarded notes.
