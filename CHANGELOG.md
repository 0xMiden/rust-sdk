...
* [FIX][cli] `miden-client account --show` now formats the vault's fungible assets through the faucet metadata resolver, so a faucet is named and its amount scaled the same way as in `notes --show` and in the transaction preview. It previously read the token config only from a faucet account the client tracks, so an asset from any other faucet was printed as raw base units next to a faucet ID prefix, even when the token symbol map named that faucet or the client had already fetched its metadata. The resolver also consults a tracked faucet account now, which is the only source of metadata for a private faucet ([#2529](https://github.com/0xMiden/rust-sdk/pull/2529)).
